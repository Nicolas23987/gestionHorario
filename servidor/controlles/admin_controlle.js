...
const authenticateToken = (req, res, next) => {
    ...
    jwt.verify(token, 'secret', (err, user) => {
        if (err) return res.status(403).json({ status: false, message: 'Token invalid' });
        
        return res.status(202).json({ status: true, message: 'Token' });
...
module.exports = {
    create_Admin, getAdmin, update_Admin, auth_admin, authenticateToken
};